...
The `Environment` manages simulation time and the synchronization between all components. We start by creating one with a step size of 5 minutes (300 seconds):
...
- **live** (via `vs.Environment.live(...)`) advances at wall-clock speed and can connect to real systems, see [Software-in-the-Loop](concepts/sil.md). Here, `sim_start` is optional and defaults to `datetime.now()`.
...
A `Trace` replays a CSV row-by-row, indexed by an offset in seconds since the trace's start. See [Signals and Datasets](concepts/signals.md) for all available signal types, the CSV schema, and how to fetch more data.
...
`add_microgrid` combines actors, dispatchables, and grid signals into one simulated system. 
...
The last ingredient is a **Controller**. 
Controllers observe the simulation at every step and can react to it.
We use the built-in `CsvLogger`, which writes the experiment configuration and full timeseries to a directory:
