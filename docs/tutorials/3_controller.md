...
A `Controller` is a class that is executed at every simulation step. It has access to the state of all microgrids and can modify their components (like dispatchables or actors).
...
*   `dispatch_states`: The states of all dispatchables (e.g., `soc` for batteries).
...
class CarbonAwareController(vs.Controller):
    ...
    def step(self, now, microgrid_states):
        for name, state in microgrid_states.items():
            ...
            mg = self.microgrids[name]
            for d in mg.dispatchables:
                if hasattr(d, "min_soc"):
                    ...
                    if carbon_intensity > self.carbon_threshold:
                        ...
                        d.min_soc = self.high_min_soc
                    else:
                        d.min_soc = self.low_min_soc
```
